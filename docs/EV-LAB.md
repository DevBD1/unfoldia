# EV Lab — historical prototype reference

This document preserves the original prototype notes. For current paths, commands and deployment settings, use the [root README](../README.md). EV source files now live in `src/labs/ev/`; old local machine paths below are historical, not setup requirements.
