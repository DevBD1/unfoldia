...
test('community invitation is available in every catalog language', () => {
  for (const copy of Object.values(catalogCopy)) {
    ...
    assert.ok(typeof copy.communityHeading === 'string' && copy.communityHeading.trim().length > 0);
  }
  ...
  assert.ok(source.includes("communityHeading.id = 'community-heading'"));
  assert.ok(source.includes("community.setAttribute('aria-labelledby', communityHeading.id)"));
  assert.ok(source.includes('community.append(communityHeading, invitation, contribute)'));
});
